...
| C, C++23 and C# fleet run/cancel/report | Implemented | The C ABI exposes blocking/async run, wait, cancel, terminal report extraction and report release (`include/kairosboot/kairosboot.h`, `src/api/fleet_run.cpp`). Report JSON is copied into immutable shared storage before a public report handle is created. The C++ wrapper owns move-only `Job`/`JobReport` handles, and the .NET wrapper uses `SafeHandle`, `Task`, `CancellationToken`, `IProgress<T>` and independent report handles. Fleet orchestration is intentionally SDK-only: the CLI exposes only frozen Fastboot commands and options. | `tests/c_api/fleet_run_test.c` covers success, failure/cancel report extraction, report independence, production preparation, 32-device execution and isolation. `tests/cxx_fleet_run_test.cpp` covers move-only lifecycle and reports after failure/cancel. `bindings/dotnet/KairosBoot.ContractTests/Program.cs` covers the public surface, cancellation drain and independent failure/cancel reports. `tests/check_cli.cmake` rejects non-Fastboot `validate`, `plan`, and `run` commands. |
...
- `device_actor`, `fleet_coordinator`, `reconnect_coordinator`,
  ...
  `cli_contract`, `cli_scripted`, and the TCP/UDP loopback suites.
- The net10.0 scripted native .NET contract suite: 522 checks, including the
...
This runtime/fleet audit does not claim the whole project is release-ready. The
current frozen inventory has no missing or partial implementation entries, and
the exact-head Darwin capture matches official Platform-Tools 37.0.1 in 37
host/TCP/UDP scenarios. Those scenarios provide official evidence for 51 of 86
required entries; full compatibility still requires the remaining official
differential and hardware acceptance evidence above.
